...
## Дефолтные логи в Kubernetes

Система логирования в Кубере построена достаточно просто - контейнер в поде пишет логи в STDOUT либо в STDERR.

Все что пишется в эти каналы вывода - будет подхвачено Docker Daemon-ом, и сложено в файл на хостовой ОС (log-file.log для примера).

Ротацией логов - также занимается Docker Daemon. Настройки ротации - там же. 

Бекенд логирования можно поменять, по умолчанию это JSON-файл, но можно писать и в Journald и в удаленную систему логирования. На практике почти всегда используется JSON как по умолчанию. 

Как это выглядит в Kubernetes:

```bash
kubectl logs -n <имя неймспейса> <имя пода>
```

Где реально на хостовой системе хранятся логи:

`/var/lib/docker/containers/<хэш образа контейнера>/хэш образа контейнера-json.log`

Это путь, куда складывает логи Docker Daemon.

А `kubectl logs` работает с другим файлом:

`/var/logs/pods/<имя неймспейса>/<имя пода>/0.log`

Это на самом деле симлинк на файл лога, который складывает Docker Daemon.

Есть еще одна директория: 

`/var/log/containers` - там тоже лежат симлинки на те же логи.

В результате логи с ноды Кубернетес можно забрать аж с трех мест в файловой системе.

Но как правило, принято забирать логи с `/var/log/pods`



Все системные логи Kubernetes можно забирать как логи подов, кроме логов kubelet и логов Docker Daemon, потому что они запущены как **сервисы** на ноде, а не как **поды**.

Эти логи можно достать из `journald` при помощи `journalctl`



Есть еще один файл - `/var/log/kubernetes.log` - в нем лежат по умолчанию логи kubelet-а, если кластер разворачивался с помощью утилиты kubeadm.

В файле `/etc/rsyslog.d/kubernetes.conf` как раз указывается, в какой файл складывать логи kubelet-а. 



## Архитектура и инфраструктура целевой системы логирования

Что глобально нужно для системы логирования в Kubernetes:

- сохранять логи подов после редеплоя (по умолчанию поды удаляются вместе со всеми данными)
- агрегировать логи со всех реплик подов приложения и показывать как логи приложения в целом
- добавлять метаинформацию по логам (нода, неймспейс, и т. д.)
- иметь возможность парсить логи - получать набор полей из логов (например Severity) и фильтровать по данным полям. 



Какая архитектура считается каноничной:

- запуск пода системы логирования на ноде (как правило в виде DaemonSet)
- агент в поде логирования собирает логи с ноды - как системные (из journald), так и логи подов - либо из файлов на ноде напрямую, либо через обращение к Kubernetes API
- собранные логи отправляются на logging backend

## ElasticSearch, FluentBit, Kibana (EFK)

FluentBit/FluentD - продукты от одного разработчика. Разница в том, что:

- FluentD - штука, написанная на более медленном языке
- FluentD - штука более масштабная
- FluentD обычно используется не с контейнерами и не в Kubernetes.



FluentBit - более легковесный. 

В настройке FluentBit - указываем сбор логов из подов, из `/var/log` и из `journald`

Поды-сборщики будут отправлять логи в ElasticSearch - базу для хранения логов.

Kibana - это UI-интерфейс для Эластика для просмотра логов, генерации дашбордов, фильтрации.



### Параметры FluentBit:

Секция `service`: общие настройки

- Flush <число> - как часто будут логи сбрасываться в Эластик (в секундах)
- Daemon Off - не запускаться в Background-режиме внутри контейнера, чтобы контейнер не завершал работу
- Log_Level info - минимальный уровень логов, который будет сбрасываться
- Parsers_File <имя файла> - файл парсеров
- HTTP_Server On - сервер, по которому контейнер будет отдавать свои метрики
- HTTP_Listen <адрес>
- HTTP_Port {{ .Values.service.port }}

Секция `inputs` - инпуты, то есть источники логов

- Name tail (или Name systemd) - хвост файла с логами либо инфо от сервиса (для systemd)
- Path <путь> - путь к логу для tail
- Parser <тип парсера> - тип парсера для tail, обычно `docker`
- Tag <тэг> - фильтр по записям и роутинг логов (`kube.*` для tail и `host.*` для systemd)
- Mem_Buf_Limit <лимит в МБ> - максимум буфера, который считывается за раз (нужно ограничение, чтобы потребление памяти контейнера с FluentBit не разрасталось)
- Skip_Long_Lines On - исключить длинные строки, которые не влезли
- Systemd_Filter - фильтр по демонам для systemd, например `_SYSTEMD_UNIT=kubelet.service` или `=docker.service`

Секция `filters` - фильтры для обогащения логов метаинформацией

- Name kubernetes - 
- Match kube.* - тег
- Merge_Log On - если внутри поля Log в логе содержится JSON - то он парсится на отдельные поля и отправляется в Эластик уже распарсенным
- Keep_Log Off - изначальное поле Log можно из лога удалять
- K8S-Logging.Parser On - указать аннотацию `fluent.io/Parser: true` - удобно, когда логи не в JSON
- K8S-Logging.Exclude On - указать аннотацию `fluent.io/Exclude: true` - логи этого пода не нужно никуда отправлять, например для тестов

Секция `outputs` - куда выводить логи

- Name es - имя вывода
- Match kube.* - тег
- Host elasticsearch-master - на какой хост посылать
- Logstash_Format On - сразу писать логи в формате Эластика
- Logstash_Prefix node - добавить префикс для логов хоста для Эластика, для kubelet лучше выставить вместо node - kube. 
- Retry_Limit False - пытаться отправить логи без лимита (постоянный ретрай, если Эластик недоступен, либо указать кол-во попыток ретрая)
- Replace_Dots On - исправить параметр, чтобы лейблы попадали в ElasticSearch не как точки (kubernetes.io.elasticsearch.*), а как нижнее подчеркивание, чтобы Эластик не считал, чтобы это вложенные друг в друга поля. Если следовать best practices и именовать лейблы без точек - такого быть не должно, но на всякий случай рекомендуется поставить.

Секция `customParsers` - кастомные парсеры

- Name docker_no_time - имя
- Format json - формат
- Time_Keep Off - сохранять таймстамп
- Time_Key time - ключ для базы
- Time_Format <формат> - формат

### **Работаем с Helm**

1. Подключаем репо `elastic`:
...
2. Получаем `values` для чарта `elastic/elasticsearch`

```bash
helm show values elastic/elasticsearch > elastic-values.yaml
```

3. Правим настройки чарта. Находим в elastic-values.yaml `volumeClaimTemplate` и меняем 30Gi на 5Gi

4. Устанавливаем ElasticSearch в кластер

```bash
kubectl create ns logging

helm upgrade -i elasticsearch elastic/elasticsearch -f elastic-values.yaml -n logging
```

5. Дождитесь пока все поды запустятся и станут Ready
...
1. По аналогии с предыдущим заданием и материалами из видео подключите Helm репозиторий fluent, который находится по адресу https://fluent.github.io/helm-charts и скачайте values чарта fluent-bit
...
2. Установите fluentbit в кластер с помощью команды

```bash
helm upgrade -i fluent-bit fluent/fluent-bit -f fluent-values.yaml -n logging
```

3. Дождитесь пока все поды запустятся и станут Ready

```bash
kubectl get pods --namespace=logging -w
```

4. Добейтесь запуска fluentbit агентов на ВСЕХ нодах кластера. Для этого отредактируйте раздел `tolerations` values файла как было показано в видео. Примените настройки в кластер.
...
1. Скачайте values чарта `elastic/kibana` по аналогии с предыдущими заданиями

```bash
helm show values elastic/kibana > kibana-values.yaml
```

2. Отредактируйте values для Kibana. Найдите секцию `ingress`. Поменяйте в ней `enabled: false` на `enabled: true`, в hosts пропишите адрес - `kibana.s<ваш номер логина>.edu.slurm.io`

3. Установите чарт в кластер как в предыдущих шагах с использованием сохраненных values в неймспейс `logging`
...
4. Проверьте в браузере, что у вас открывается интерфейс Kibana по адресу (в режиме "инкогнито") `kibana.s<ваш номер логина>.edu.slurm.io`



## Работа с Kibana

Основное рабочее пространство - в разделе **Discovery**.

Сначала выбирается **index pattern** (например `kube-*`).

Вверху справа выбирается диапазон времени, за который отображаются логи.



В поле **Search** вводятся поисковые запросы. Запросы можно писать на современном языке запросов (Kibana Query Language), либо на устаревшем Lucene SQL.



Все сообщения в логах представляют набор полей.

Среди этих полей есть поле `Log` - внутри этого поля собственно и лежит сообщение лога, все остальные поля являются метаинформацией. 



### Примеры запросов

В поле поиска  можно использовать условные операторы   AND, OR и NOT, и регулярные выражения. Группировать можно скобками. Можно использовать операторы <>.

`kubernetes.pod_name: kibana and res.statusCode: 200` - выведет все логи успешных запросов к подам Кибаны

В левой панели перечислены все поля, и можно найти интересующее поле. Выбрав его - будут показываться только значения этого поля. В Selected Fields можно добавить несколько полей.

Обычного этого более чем достаточно для разработчиков, чтобы работать с логами.  



### Дашборды

Для того, чтобы качественно работать с дашбордами - логи надо парсить. Лучше всего это делать, научив приложение генерить логи в формате JSON.

Конструктор дашбордов работает по одинаковому принципу - выбирается тип дашбордов, на нем выбираются объекты, на объекты перетягиваются поля и далее настраиваются параметры конкретного дашборда типа цвета, временного интервала, значений по осям для графиков и т.д.



## Loki, Promtail, Grafana

Promtail - аналог FluentBit - это под-сборщик логов. Также разворачивается как DaemonSet.

Loki - это БД для хранения логов. Гораздо более легковесный, чем ElasticSearch, лучше чувствует себя внутри кластера Kubernetes, но менее производительный и не подходит для Highload систем. Подходит для небольших кластеров, типа dev/stage. Намного проще настраивать и использовать, чем Эластик.

Графана - визуализатор.

В этом стеке удобно то, что и метрики и логи можно просматривать из одной точки - из Графаны.



В Loki  довольно обширные возможности по настройке из Helm-чарта, по умолчанию он будет ставить локальную DynamoDB и хранить логи там. Можно также подключить внешнюю Amazon DynamoDB или S3.

Задаются периоды хранения и ротации логов.

Loki умеет работать с несколькими инстансами и инстансы сами договариваются между собой, кто из них будет мастером. 



Параметры Promtail, на которые стоит обратить внимание:

- `lokiAddress` - собственно адрес API Loki-сервера, куда будут пушами отправляться логи
- `tolerations` - по умолчанию вписаны для мастеров, чтобы логи собирались с мастеров. Для других типов нод, типа ингрессов - либо дописать, либо убрать детализацию для мастеров, чтобы любые taints игнорировались.



В Графане в качестве источника данных выбирается Loki. Если Loki и Grafana в разных неймспейсах одного кластера, то нужно в поле адреса указать еще имя неймспейса, например `http://loki.logging:3100`.



Loki создавался по принципу быть максимально похожим на Prometheus, только для логов, а не для метрик. Соответственно, язык запросов похож.

### **Устанавливаем Loki**

!!! ВНИМАНИЕ: Работы продолжаем на стенде из предыдущего шага

1. Подключаем репо grafana:
...
2. Получаем `values` для чарта `grafana/loki`

```bash
helm show values grafana/loki > loki-values.yaml
```

3. Правим настройки чарта. Находим в loki-values.yaml `persistence` и меняем 10Gi на 5Gi

4. Устанавливаем ElasticSearch в кластер

```bash
helm upgrade -i loki grafana/loki -f loki-values.yaml -n logging
```
...
1. Скачайте values чарта `grafana/promtail`

```bash
helm show values grafana/promtail > promtail-values.yaml
```

3. Правим настройки чарта.
...
```bash
helm upgrade -i promtail grafana/promtail -f promtail-values.yaml -n logging
```
...
1. Скачайте values чарта `grafana/grafana`

```bash
helm show values grafana/grafana > grafana-values.yaml
```

2. Отредактируйте values для grafana. Найдите секцию `ingress`. Поменяйте в ней `enabled: false` на `enabled: true`, в hosts пропишите адрес - `grafana.s<ваш номер логина>.edu.slurm.io`

Находим `persistence` и меняем 10Gi на 5Gi.

Раскомментировать `adminPassword`

3. Создать namesapce `monitoring` и установите чарт в кластер как в предыдущих шагах с использованием сохраненных values в неймспейс `monitoring`

```bash
kubectl create ns monitoring

helm upgrade -i grafana grafana/grafana -f grafana-values.yaml -n monitoring

kubectl get pods --namespace=monitoring -w
```

4. Проверьте в браузере, что у вас открывается интерфейс grafana по адресу (в режиме "инкогнито") `grafana.s<ваш номер логина>.edu.slurm.io`

5. Добавьте датасурс Loki.

![img](https://api.edu.slurm.io/uploads/43875cfa1dbd979094aee43de2651c7d.png)

![img](https://api.edu.slurm.io/uploads/66adbcfdbb02722d826762d3448bc162.png)

![img](https://api.edu.slurm.io/uploads/65cc9bfa8f360aca95551261830c116e.png)

И нажимаем "**Save & Test**"

6. Проверьте. что вы можете просматривать логи в Grafana как показано в видео.

Заходите в **Explore** и в поле **Log Labale** наберите - **{namespace="ingress-nginx"} |= "POST"** 

Далее нажмите **Run Query**
...
1. Разработчик должен иметь возможность самостоятельно создавать алерты для своих приложений в кластере. Например можно использовать объект **Rules** из Prometheus Operator. 
...
1. Алерты должны приходить тем кому они нужны и тем кто будет что то делать с проблемой. Нет смысла слать все алерты дежурному, если он ничего с ними не будет предпринимать. Очень быстро каналы алертов превращаются в бесконечную спам свалку.
2. Каждый пришедший алерт должен обрабатываться. Если по алерту не требуется каких то ручных действий нет смысла отсылать его в канал мониторинга. Возможно достаточно ограничиться электронным письмом или каким то другим каналом передачи с минимальным приоритетом.
3. Нужно различать понятие лога (событие произошедшее в системе) и Stacktrace (необработанной ошибки в коде приложения). Логи нужно парсить и по ним составлять необходимые графики и алерты. Стактрейсы нужно собирать в специализированные системы (например **Sentry**) и работать с ними отдельно.
4. **Grafana** позволяет на одном графике сочетать данные из нескольких источников. Например данные мониторинга и системы логирования. Таким образом можно связывать например графики ошибок в запросах из мониторинга и сообщения логов, которые эту ошибку описывают.
5. Не имеет смысла собирать все возможные метрики и все возможные сообщения логов в максимальном уровне логирования. Эти данные чаще всего не используются, но при этом очень быстро приводят к разрастанию систем мониторинга и логирования, что может привести к их неработоспособности. Выделите набор нужных метрик, и оптимальный уровень логирования. Обсуждайте с разработкой целесообразность добавления новых метрик.